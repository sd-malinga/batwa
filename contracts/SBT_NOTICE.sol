...
// File: Police/SBT2.sol

pragma solidity ^0.8.20;

contract RedWallet_Test is ERC721, ERC721URIStorage, ERC721Burnable, Ownable {
  ...
  constructor() ERC721("RedWallet_Test", "RWT") Ownable(msg.sender) {}

  function safeMint(BatchMint[] memory _data) public onlyOwner {
    for (uint256 i = 0; i < _data.length; i++) {
      address to = _data[i].to;
      string memory uri = _data[i].uri;
  ...
  // Modifying _update to only allow the owner to transfer tokens
  function _update(
    address to,
    uint256 tokenId,
    address auth
